...
- The Project A Plan, due when the [Calendar](calendar.qmd) says it is.
- The Project A Portfolio and Presentation, due when the [Calendar](calendar.qmd) indicates.
...
On our [432-data page](https://github.com/THOMASELOVE/432-data), you will find a pair of Quarto templates: 

- [this one is for the Project A Plan](https://raw.githubusercontent.com/THOMASELOVE/432-data/refs/heads/master/data/432_projectA_plan_template.qmd), and 
- [this one is for the Project A Portfolio](https://raw.githubusercontent.com/THOMASELOVE/432-data/refs/heads/master/data/432_projectA_portfolio_template.qmd). 
...
A demonstration Project A, built by Professor Love, is [available here](432_projectA_demo.qmd), where you can view the HTML and download the Quarto code. 
...
1.  **Shared with the World**. The data must be available to you, and shared with me and everyone else in the world (without any identifying information) as a well-tidied file by the time you submit your Project A Plan. If the data is from another source, the source (web or other) must be completely identified to me. Ongoing projects that require anyone’s approval to share data are not appropriate for Project A.
    ...
    -   You will need to use meaningfully different data sets in 432 Projects A
        and B.
...
2.  **Size**.
    -   A **minimum** of 150 complete observations are required on each
        variable. It is fine if there are some missing values, as well,
        so long as there are at least 150 rows with complete
        observations on all variables you intend to use in each model.
    -   The **maximum** data set size is 2000 observations, so if you
...
4.  **Inputs**. You will need at least four regression inputs
    ...
    -   At least one of the four must be quantitative (a variable is
        **not** quantitative for this purpose unless it has more than 10
...
7.  The [500 Cities and PLACES data portal](https://chronicdata.cdc.gov/browse?category=500+Cities+%26+Places&sortBy=newest&utf8), most probably focusing on the [County-level data](https://chronicdata.cdc.gov/500-Cities-Places/PLACES-Local-Data-for-Better-Health-County-Data-20/swc5-untb).
...
- a Quarto (`.qmd`) file containing one unnumbered, and 10 numbered sections built using the [template](#templates) we have provided.
...
All variables in your tidy data set, and in your codebook in Section 5 should fall into one of four roles. Don't include other things in your tidy data set.
...
- a Quarto (`.qmd`) file containing containing one unnumbered, and 13 numbered sections (10 of which come from the Plan) built using [the template](#templates) for the portfolio that we have provided for you
...
Completing all of what is listed above for Section 8 appropriately is the best way to receive an excellent grade on Project A. The [Project A demonstration project](https://thomaselove.github.io/432-2025/projA.html#demonstration-project) provides R code to help illustrate how you can accomplish those tasks.
...
### EXTRA (potential section 9.9)

Completing all of what is listed above for Section 9 appropriately is the best way to receive an excellent grade on Project A. The [Project A demonstration project](https://thomaselove.github.io/432-2025/projA.html#demonstration-project) provides R code to help illustrate how you can accomplish those tasks.

Some of you may be eager to show some *additional* analytic facility. If you have successfully done **all** of the things provided in Section 9 (specifically subsections 9.1 - 9.8) of the [Project A demonstration project](https://thomaselove.github.io/432-2025/projA.html#demonstration-project), and still want to do more, choose between the following two options and create a new section 9.9, as indicated. You are permitted to include both a section 8.9 and 9.9 in your Project Portfolio.

Three key points to make at the start about this extra work...

- If you have meaningful problems in sections 9.1 - 9.8, I won't even look at your section 9.9.
- Although section 9.9 can only help your grade, this is completely optional, and is not required to get an A grade on the Project. 
- You may choose **exactly one** of the two options below. Do not present both, or I will ignore both.

Your choices (1 and 2) for section 9.9 of your portfolio are as follows...

1. **If** in your logistic regression you have **more than 5% of cases** with missing data in your predictors, create a new section 9.9 called "Comparing Imputation Strategies", where you show the simple imputation model, and then either:
...
2. After your logistic regression modeling, create a new section 9.9 called "A New Model X" where you do the following:
...
slide presentation (not to exceed **four** minutes) of what you feel is **the
...
-   If you have a partner, you should record a single presentation with
    ...
    the total time should not exceed **four** minutes.
